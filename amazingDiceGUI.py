...
# MARK: - COIN DISPLAY (top left on all screens except login)

def placeCoinDisplay(window, username):
    """Place coins (amount + icon) in top-left of window. Amount to the left of icon."""
    try:
        coins = ad.getCoins(username) if username != 'Guest' else 0
    except Exception:
        coins = 0
    coinStr = f"{coins:,}"
    frame = customtkinter.CTkFrame(window, fg_color="transparent")
    frame.place(relx=0.02, rely=0.02, anchor="nw")
    amountLabel = customtkinter.CTkLabel(frame, text=coinStr, font=("Arial Bold", 15))
    amountLabel.pack(side="left", padx=(0, 4))
    coinImg = customtkinter.CTkImage(
        light_image=Image.open("images/icons/coinIcon.png"),
        dark_image=Image.open("images/icons/coinIcon.png"),
        size=(22, 22),
    )
    iconLabel = customtkinter.CTkLabel(frame, image=coinImg, text="")
    iconLabel.image = coinImg  # keep reference so it doesn't get garbage collected
...
def shop(username, guestMode, settings): # unfinished so mostly not tagged or accesible and does absolutely nothing lmao
    ...
    coinAmountLabel = placeCoinDisplay(shopWindow, username)
    ...
    # Items per row (wrap happens every this many)
    shopItemsPerRow = 3
    shopItemWidth = 200
    shopItemPadx = 10
    shopRowPadx = 10
    # Width to fit all 3 cards with plenty of room (scrollbar + padding)
    scrollFrameWidth = shopItemsPerRow * (shopItemWidth + shopItemPadx * 3) + shopRowPadx * 3 + 300
    scrollFrame = customtkinter.CTkScrollableFrame(shopWindow, width=scrollFrameWidth, height=400)
    scrollFrame.pack(pady=10)

    def getSafeCoins():
        try:
            return ad.getCoins(username) if username != 'Guest' else 0
        except Exception:
            return 0

    def getSafeInventory():
        try:
            return ad.getInventory(username) if username != 'Guest' else []
        except Exception:
            return []

    def updateCoinsDisplay():
        coinAmountLabel.configure(text=f"{getSafeCoins():,}")
    ...
    def renderShopItems():
        clearShopItems()
        coins = getSafeCoins()
        inventory = getSafeInventory()

        rowFrame = None
        for i, item in enumerate(itemList["items"]):
            # Wrap to a new row every shopItemsPerRow items
            if i % shopItemsPerRow == 0:
                rowFrame = customtkinter.CTkFrame(scrollFrame)
                rowFrame.pack(pady=10, padx=shopRowPadx, side='top')

            # Fixed width so all cards are equal and the last one isn't squished
            itemFrame = customtkinter.CTkFrame(rowFrame, fg_color=("gray85", "gray25"), width=shopItemWidth)
            itemFrame.pack(pady=10, padx=shopItemPadx, side='left', ipadx=50, ipady=5, anchor='n')
            itemFrame.pack_propagate(False)  # keep fixed width
            ...
            owned = item in inventory
            affordable = itemList['items'][item]['cost'] <= coins

            if owned:
                buyableColor = 'gray'
                state = 'disabled'
                ownedLabel = customtkinter.CTkLabel(itemFrame, text='You already have this item')
                ownedLabel.pack(pady=3)
            elif affordable:
                buyableColor = 'green'
                state = 'normal'
            else:
            ...
            def buyPressed(itemId=item):
                ad.buy(username, itemId, itemList)
                updateCoinsDisplay()
                renderShopItems()

            buyButton = customtkinter.CTkButton(itemFrame, text='Buy', fg_color=buyableColor, state=state, command=buyPressed)
...
def passwordChange(username, guestMode, settings): # CHANGE UR PASSWORD WITH THIS ONE SIMPLE TRICK 😮😮😮😮😮
    ...
    placeCoinDisplay(changeWindow, username)
...
def settingsWindow(username, guestMode, settings):
    ...
    placeCoinDisplay(settingsWindow, username)
...
def match(botOrNot, difficulty, username, username2, guestMode, settings): # botOrNot is the best var name ever ( also i hate ts code vro fr )
    ...
    placeCoinDisplay(gameWindow, username)
    ...
    def startPressed(): # TERRIBLE CODE DONT LOOK AT THIS 
        ...
        for i in range(0, 5): # 5 rounds
            ...
            rolls1, rolls2, rollTotal1, rollTotal2, player1Starts, startRoll1, startRoll2 = ad.playRound(ad.getSize(username), ad.getSize(username2) if not botOrNot else 6, ad.getDifficultyInt(difficulty) if botOrNot else 0, username, botOrNot)  # rolls 3 times and stores it in a list. gets the roll totals and gets who starts + their starting rolls
...
# MARK: - WIN
        ...
        if playerScore2 < playerScore1: # IF YOU WON OVERALL
            winReward = ad.getWinReward(difficulty, username)
            if botOrNot: # YOU WON VS BOT
                ...
                rewardLabel.place(relx=0.5, rely=0.45, anchor="center")  # put below info label
...
# MARK: - LOSE
            ...
            if username != 'Guest':  # all the stat stuff i dont want to tag it all you read the last bit 
                ...
                if username2 == 'Bot':
                    ...
                    userData['users'][username2]['stats']['netWorth'] += winReward
...
def pvp(username, settings):
    ...
    placeCoinDisplay(pvpWindow, username)
...
def bot(username, guestMode, settings):
    ...
    placeCoinDisplay(botWindow, username)
    ...
    inventory = ad.getInventory(username)
    for item in inventory:
        ...
        if item == 'luckyCharm':
            difficultyAdditive += 1
        if item == 'riskTaker':
            difficultyAdditive += 1.5
        if item == 'diceMaster':
            difficultyAdditive += 2
...
def matchWindow(username, guestMode, settings):  # defines match window function that takes username and guest mode status
    ...
    placeCoinDisplay(matchWindow, username)
...
def viewStatsWindow(stats, username, settings):  # defines stats window function that takes stats dictionary and username
    ...
    placeCoinDisplay(statsWindow, username)
...
def achievementsWindow(username, settings):
    ...
    placeCoinDisplay(achievementsWindow, username)
...
def menu(guestMode, username, settings):  # defines menu window function that takes guest mode status and username
    ...
    placeCoinDisplay(menuWindow, username)
...
def login(savedUsername, savedPassword):  # defines login window function
    ...
    def loginPress():  # defines function for login button
        ...
        if username == savedUsername and not password: # if trying to use saved username but with empty password field will attempt to use keyring auto login
            try: # try to get the password from keyring
                ...
                if storedPassword and ad.login(username, storedPassword): # if password is found and is valid
                    ...
                    errorCounters["keyringFailure"] += 1
                    messageLabel.configure(text=f"Please enter your password ({errorCounters['keyringFailure']})", text_color="red") # shows error for failed login
            except:
                errorCounters["keyringFailure"] += 1
                messageLabel.configure(text=f"Please enter your password ({errorCounters['keyringFailure']})", text_color="red")
        ...
        if not username or not password:  # checks if fields are empty
            ...
            messageLabel.configure(text=f"Please fill in all fields ({errorCounters['emptyFields']})", text_color="red")  # shows error for empty fields
        ...
        if ad.login(username, password):  # attempts to log in with credentials normally
            ...
            messageLabel.configure(text=f"Login failed. Check username and password. ({errorCounters['invalidCredentials']})", text_color="red")  # shows error for failed login
        ...
        def autoLoginPressed():
            try:
                ...
                if passwordField.get(): # if entered password
                    ...
                    errorCounters["quickLogin"] += 1
                    messageLabel.configure(text=f"Quick login failed. Please enter your password. ({errorCounters['quickLogin']})", text_color="red")
            except:
                errorCounters["quickLogin"] += 1
                messageLabel.configure(text=f"Quick login failed. Please enter your password. ({errorCounters['quickLogin']})", text_color="red")
...
def createAccountWindow():  # defines create account window function
    ...
    def createAccountPressed():  # defines function for create account button
        ...
        if not username or not password:  # checks if fields are empty
            errorCounters["emptyFields"] += 1
            messageLabel.configure(text=f"Please fill in all fields ({errorCounters['emptyFields']})")  # shows error for empty fields
        ...
        if not (len(password) >= 12 and  # checks password requirements
            ...
            errorCounters["passwordRequirements"] += 1
            messageLabel.configure(text=f"Password must be at least 12 characters and contain uppercase lowercase numbers and special characters ({errorCounters['passwordRequirements']})")  # shows error for invalid password
            return  # exits function
        ...
        if ad.createAccount(username, password):  # attempts to create account
            ...
            errorCounters["usernameExists"] += 1
            messageLabel.configure(text=f"Username already exists, please choose another ({errorCounters['usernameExists']})")
