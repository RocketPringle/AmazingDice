...
def saveCoins(username, coins):
    data = loadUsers()
    currentData = data['users'][username]
    currentData['coins'] = coins
    data['users'][username] = currentData
    saveUsers(data)

# MARK: - BUY

def buy(username, item, itemList):
    print(item)
    cost = itemList['items'][item]['cost']
    inventory = getInventory(username)
    coins = getCoins(username)
    if coins >= cost:
        if item not in inventory:
            inventory.append(item)
            saveInventory(username, inventory)
            saveCoins(username, coins - cost)
            print('bought')
        else:
            print('have')
    else:
...
def getInventory(username):
    data = loadUsers()
    return data['users'][username]['inventory']

# MARK: - SAVE INVENTORY

def saveInventory(username, inventory):
    data = loadUsers()
    currentData = data['users'][username]
...
def addNewUser(username, password):  # defines function to add new user to system
    ...
    data['users'][username] = {  # creates new user entry
        ...
        ],  # initializes empty achievements list
...
def diceRoll(baseRoll, size, difficulty, botDouble):
    print(f'bot double {botDouble}')
    return (random.randint(baseRoll,size) + difficulty) * botDouble # THIS FUNCTUIOIN DOESNT MATTER ITS SO SHORT AND DUMB
...
def getWinReward(difficulty, username):
    ...
    mult = 1
    for item in inventory:
        if item == 'gamblingAddiction':
            mult *= 2
        if item == 'highStakes':
            mult *= 1.5
        if item == 'riskTaker':
            mult *= 2
        if item == 'fortuneFavour':
            mult *= 3
    if difficulty == 'Easy':
        return int(500 * mult)
        ...
        return int(1500 * mult)
    else:
        return int(1000 * mult)

# MARK: - PLAY ROUND

def playRound(size, size2, difficulty, username, botOrNot):
    startRoll1, startRoll2 = 0,0 # set to same defaul ig idk INITIALISE THATS THE WORKD
    bonus = 0
    inventory = getInventory(username)
    rollMult, botRollMult = 1,1
    minRoll, botMinRoll = 1,1
    for item in inventory:
        if item == 'stonks':
            bonus += 1
        if item == 'gamblingAddiction' and botOrNot:
            botRollMult *= 2
            print('sdoihfouisdhfiudshfoiudshfidddddddddddddddddddddddddddddd')
        if item == 'luckyCharm':
            bonus += 2
            difficulty += 1
        if item == 'highStakes':
    ...
    while startRoll1 == startRoll2: # IF EQUAL SO DRAWS (IKTS NOT WHILE TRUE YAAAAYAYAYA) <- EXTRA POINTS FOR THAT PLS 🙏🙏🙏🙏
        startRoll1, startRoll2 = diceRoll(minRoll, size, bonus, rollMult), diceRoll(botMinRoll, size2, difficulty, botRollMult) # ROLS FOR IRSTR THIS IS DUMB WHO CARES WHO;S FIRST AAA
    rolls1, rolls2 = [], [] # INITISALKISE
    for i in range(0, 3): # 3 rolls each
        rolls1.append(diceRoll(minRoll, size, bonus, rollMult)) # roll for player 1
        rolls2.append(diceRoll((botMinRoll if botOrNot else 1), size2, difficulty, botRollMult)) # roll for player 2, difficulty included as bots can only be player 2 and so is botOrNot, need to add opponent min roll featuree to btw ooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooaaaaaaaaaaaaaaaaaaaaaaaaaaaa
    print(rolls2)
